...
describe('useEditorSlashInsertion', () => {
  ...
  it('inserts ordered list and consumes slash token range', () => {
    ...
    const insertion = useEditorSlashInsertion({
      getEditor: () => editor,
      currentTextSelectionContext: () => ({ text: '/li', nodeType: 'paragraph' }),
  ...
  it('inserts structural content when slash context is absent', () => {
    ...
    const insertion = useEditorSlashInsertion({
      getEditor: () => editor,
      currentTextSelectionContext: () => ({ text: '/h2', nodeType: 'paragraph' }),
      readSlashContext: () => null
    })

    const ok = insertion.insertBlockFromDescriptor('header', { level: 2 })

    expect(ok).toBe(true)
    expect(chain.insertContent).toHaveBeenCalledWith({ type: 'heading', attrs: { level: 2 }, content: [] })
  ...
  it('returns false when editor is unavailable', () => {
    const insertion = useEditorSlashInsertion({
      getEditor: () => null,
      currentTextSelectionContext: () => ({ text: '', nodeType: 'paragraph' }),
      readSlashContext: () => null
    })
