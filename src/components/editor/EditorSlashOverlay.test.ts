...
vi.mock('./EditorSlashMenu.vue', () => ({
  default: defineComponent({
    ...
    setup(_, { emit }) {
      return () => h('button', { class: 'slash-overlay-stub', onClick: () => { emit('update:index', 3); emit('select', { id: 'quote', label: 'Quote', type: 'quote', data: {} }) } }, 'slash')
    }
  })
}))
...
describe('EditorSlashOverlay', () => {
  it('forwards slash index and select events', () => {
